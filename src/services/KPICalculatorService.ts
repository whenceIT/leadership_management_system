...
export class KPICalculatorService {
  ...
  private async getBranchManagerKPIScores(userId: number, positionId: number): Promise<any[]> {
    const results: any[] = [];
    const endpoints = [
  ...
  public async markscore(userId: number, positionId: number, kpi_name: string): Promise<any[]> {
    ...
    // Branch Manager
    if (positionId === 5) {
        const branchManagerResults = await this.getBranchManagerKPIScores(userId, positionId);
        results.push(...branchManagerResults);
    }
